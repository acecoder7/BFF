...
// import pkg1 from '@web3auth/modal';
// const { Web3Auth } = pkg1;
// import pkg from '@web3auth/solana-provider';        
// const SolanaWallet = pkg.SolanaWallet;
...
export const login = async (req, res) => {
  try {
    ...
    // const publicKey = await solanaWallet.getPublicKey();
    // const solanaPublicKey = publicKey.toBase58();
    // if (user.solanaPublicKey !== solanaPublicKey) {
    ...
    res.setHeader(
      "Set-Cookie",
      cookie.serialize("codelet", token, {
        httpOnly: true,
...
export const whoami = async (req, res) => {
  try {
    ...
    const token = cookie.split(";").find((c) => c.trim().startsWith("codelet="));
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    const resetPasswordToken = user.getResetPasswordToken1();
